...
import '../../../../core/services/offline_bible_service.dart';
...
class _ChapterReaderPageState extends ConsumerState<ChapterReaderPage> {
  ...
  void initState() {
    ...
    _currentBibleId = PopularTranslations.getOfflineId(actualBibleId) ?? actualBibleId;
    print('INIT: Bible ID set to $_currentBibleId');
    ...
    ref.listenManual(bibleDataProvider, (previous, next) {
      ...
      if (prevId != nextId) {
        final prevOffline = PopularTranslations.getOfflineId(prevId ?? 'kjv') ?? 'kjv';
        final nextOffline = PopularTranslations.getOfflineId(nextId ?? 'kjv') ?? 'kjv';
        
        print('LISTENER: Bible changed from $prevOffline to $nextOffline');
  ...
  String _getChapterKey(int chapter) => '${_bookAbbr}.$chapter';
  ...
  Future<void> _loadChapterContent(int chapter) async {
    ...
    print('LOADING: Bible=$bibleId, Book=$bookId (not $_bookAbbr), Chapter=$chapter');
    
    // Already cached or loading
    if (_chapterCache.containsKey(cacheKey) || _loadingChapters.contains(cacheKey)) {
      print('Already cached/loading: $cacheKey');
      return;
    }
    ...
    try {
      ...
      print('Trying DirectBibleLoader with: bibleId=$bibleId, bookId=$bookId, chapter=$chapter');
      
      var content = await DirectBibleLoader.getChapter(bibleId, bookId, chapter);
      if (content != null && content.isNotEmpty) {
        print('✓ Loaded from DirectBibleLoader');
        if (mounted) {
          setState(() {
      ...
      print('Direct loader failed for $bibleId, trying KJV fallback...');
      content = await DirectBibleLoader.getChapter('kjv', bookId, chapter);
      if (content != null && content.isNotEmpty) {
        print('✓ Loaded from KJV fallback');
        if (mounted) {
          setState(() {
      ...
      print('Critical error loading chapter: $e');
      if (mounted) {
        setState(() {
  ...
  void _saveReadingPosition() {
    print('SAVING READING POSITION: ${_bookName} ${_currentChapter}');
    ...
    print('Saving to ContinueReadingService:');
    print('  bookId: ${widget.bookId}');
    print('  bookName: ${_bookName}');
    print('  chapter: ${_currentChapter}');
    print('  bibleId: ${bibleId}');
    print('  bibleName: ${bibleName}');
  ...
  void didChangeDependencies() {
    ...
    final offlineId = PopularTranslations.getOfflineId(currentBibleId) ?? currentBibleId;
    
    if (_currentBibleId != offlineId) {
      print('DID CHANGE DEPENDENCIES: Bible changed from $_currentBibleId to $offlineId');
      setState(() {
        _chapterCache.clear();
  ...
  Widget build(BuildContext context) {
    ...
    print('BUILD: CurrentBible.id = $currentBibleId');
    
    // Check if Bible changed and reload
    if (_currentBibleId != currentBibleId) {
      print('BUILD: Bible changed from $_currentBibleId to $currentBibleId - RELOADING');
  ...
  void _toggleTheme() {
    final settings = ref.read(settingsProvider);
    final newMode = settings.isDarkMode ? ReadingMode.day : ReadingMode.night;
  ...
  Future<void> _bookmarkCurrentVerse() async {
    try {
      ...
      final already = await VerseStorageService.isBookmarked(verseId);
      if (already) {
