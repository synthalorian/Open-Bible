...
class _VerseWidgetState extends State<VerseWidget> {
  ...
  Future<void> _loadState() async {
    try {
      print('Loading state for verse: ${widget.verseId}');
      
      final highlights = await VerseStorageService.getHighlights();
      final highlightVerse = highlights[widget.verseId];
      final highlight = highlightVerse?.highlightColor;
      print('  Highlight: $highlight');
      
      final bookmarked = await VerseStorageService.isBookmarked(widget.verseId);
      print('  Bookmarked: $bookmarked');
      
      final note = await VerseStorageService.getNote(widget.verseId);
      print('  Note: $note');
      ...
      if (mounted) {
        setState(() {
          ...
          _highlightText = highlightVerse?.highlightText;
      ...
      print('Error loading verse state: $e');
    }
  }
  
  @override
  Widget build(BuildContext context) {
    final highlightValue = _highlightColor != null 
        ? HighlightColors.getColorValue(_highlightColor!) 
        : null;
      ...
      builder: (context) => _VerseActionsSheet(
        ...
        onStateChanged: ({
          ...
          setState(() {
            ...
            if (highlightText != null || highlight == '') _highlightText = highlightText;
          });
        },
      ),
    );
