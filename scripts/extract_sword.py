#!/usr/bin/env python3
"""
Extract Bible from Sword module to JSON.
Usage: python extract_sword.py <module_path> <module_name> <output_name>
"""
...
def extract_bible(module_path, module_name, bible_id, bible_name, abbreviation):
    print(f"Extracting {abbreviation} from {module_path}...", flush=True)
    
    library = SwordModules(module_path)
    library.parse_modules()
    bible = library.get_bible_from_module(module_name)
    ...
    output_file = f"/home/synth/projects/open-bible/assets/bible_data/{bible_id}_bible.json"
    ...
    print(f"\n✓ Complete! {len(bible_data['books'])} books, {total_chapters} chapters, {total_verses:,} verses")
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python extract_sword.py <module_path> <module_name> <bible_id> <bible_name> <abbreviation>")
        sys.exit(1)
    
    module_path = sys.argv[1]
    module_name = sys.argv[2]
    bible_id = sys.argv[3]
    bible_name = sys.argv[4]
    abbreviation = sys.argv[5]
    
    extract_bible(module_path, module_name, bible_id, bible_name, abbreviation)
